...
struct indexer_impl<std::integer_sequence<Tn, Ns...>> {
  ...
  template <place_id I>
  __host__ __device__ static constexpr Tn range(std::integral_constant<place_id, I>) {
    return select_value<I, extents>::value; // select_indexed_value<I, Tn,
                                            // Ns...>::value;
  }
  template <std::size_t... Is, typename... Args>
  __host__ __device__ static constexpr Tn offset_impl(std::index_sequence<Is...>, Args &&... args) {
    Tn idx = 0;
...
struct vec_view<T, std::integer_sequence<Tn, Ns...>> : identity_vec,
  ...
  __host__ __device__ explicit constexpr vec_view(T *ptr) : _data{ptr} {}
...
public:
  /// expose internal
  __host__ __device__ constexpr auto data() noexcept -> T * { return _data; }
  __host__ __device__ constexpr auto data() const noexcept -> const T * { return _data; }
  ...
  __host__ __device__ constexpr R dot(vec_impl<TT, extents> const &o) const noexcept {
    R res{0};
    for (int i = 0; i < extent; ++i)
      res += _data[i] * o.val(i);
    return res;
  ...
  template <typename TT, typename R = std::common_type_t<T, TT>>
  __host__ __device__ constexpr R dot(vec_impl<TT, extents> &&o) const noexcept {
    R res{0};
    for (int i = 0; i < extent; ++i)
      res += _data[i] * o.val(i);
    return res;
  ...
  // vector
  template <typename TT, typename R = std::common_type_t<T, TT>>
  __host__ __device__ constexpr auto operator+(vec_impl<TT, extents> const &o) const noexcept {
    vec_impl<R, extents> r{};
    for (int i = 0; i < extent; ++i)
  ...
  template <typename TT, typename R = std::common_type_t<T, TT>>
  __host__ __device__ constexpr auto operator+(vec_impl<TT, extents> &&o) const noexcept {
    vec_impl<R, extents> r{};
    for (int i = 0; i < extent; ++i)
      r.val(i) = _data[i] + o.val(i);
    return r;
  ...
  // vector
  template <typename TT, enable_if_t<std::is_convertible<T, TT>::value> = 0>
  __host__ __device__ constexpr vec_impl &operator+=(vec_impl<TT, extents> const &o) noexcept {
    for (int i = 0; i < extent; ++i)
      _data[i] += o.val(i);
    return *this;
  }
  template <typename TT, enable_if_t<std::is_convertible<T, TT>::value> = 0>
  __host__ __device__ constexpr vec_impl &operator+=(vec_impl<TT, extents> &&o) noexcept {
    for (int i = 0; i < extent; ++i)
      _data[i] += o.val(i);
    return *this;
  }
