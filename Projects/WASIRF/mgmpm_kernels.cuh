  ...
  if (blockno < blockCount) {
    ...
    for (int cidib = threadIdx.x % 32; cidib < g_blockvolume; cidib += 32) {
      ...
      if (mass > 0.f) {
        ...
        float flumex = 24.0 / g_length; // Actually 12m, added run-in/out
        float flumey = 1.22 / g_length; // 1.22m Depth
        float flumez = 0.91 / g_length; // 0.91m Width
        ...
        struct_dim[0] = ((12/12)*0.3048f) / g_length;
        struct_dim[1] = ((24/12)*0.3048f) / g_length;
        struct_dim[2] = ((12/12)*0.3048f) / g_length;
        struct_dim[0] = (0.25f) / g_length;
        struct_dim[1] = (0.25f) / g_length;
        struct_dim[2] = (0.25f) / g_length;
        vec3 struct_pos; //< Position of structures in [1,1,1] pseudo-dimension
        struct_pos[0] = (6 + 6) / g_length + offset;
        struct_pos[1] = (0.05) / g_length + offset;
