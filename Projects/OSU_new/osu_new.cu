        ...
        if (it->value.IsArray()) {
          ...
          for (auto &model : it->value.GetArray()) {
            ...
            for (int i = 0, size = mn::config::g_max_active_block; i < size; i++) {
              graph.push_back(std::array<float, 7>{0.f,0.f,0.f,0.f,0.f,0.f,0.f});
            }
    ...
    } ///< End models parsing
    {
      ...
      if (it != doc.MemberEnd()) {
        if (it->value.IsArray()) {
          ...
          for (auto &target : it->value.GetArray()) {
            ...
            int blockbits = mn::config::g_blockbits;
            // Scale to domain ([1m])
            float off = dx * ((float)(bc * (1 << blockbits)));
            /// ------------
            /// Target
            // Retrieve dimensions of target volume from JSON
            mn::vec<float, 3> h_point_a, h_point_b;
            for (int d = 0; d < 3; ++d) {
              // Pull position points from JSON (m)
              h_point_a[d] = target["point_a"].GetArray()[d].GetFloat();
              h_point_b[d] = target["point_b"].GetArray()[d].GetFloat();

              h_point_a[d] = h_point_a[d] / length + off;
              h_point_b[d] = h_point_b[d] / length + off;
            }
            fmt::print("About to set up target in gmpm.cu\n");
            
            float target_freq = target["freq"].GetFloat();
            fmt::print("target_freq {} \n", target_freq);
            auto initGridTarget = [&](auto &h_gridTarget, 
                                      auto &h_point_a, auto &h_point_b, float target_freq) {
              benchmark->initGridTarget(h_gridTarget, h_point_a, h_point_b, target_freq);
            };

            /// Instantiate zero'd initial grid target on host (JB)
            std::vector<std::array<float, 10>> h_gridTarget;
            for (int i = 0, size = mn::config::g_target_cells; i < size; i++) {
              h_gridTarget.push_back(std::array<float, 10>{0.f,0.f,0.f,
                                                          0.f,
                                                          0.f,0.f,0.f,
                                                          0.f,0.f,0.f});
            }
            fmt::print("Set up target in gmpm.cu\n");

            /// Write initial target to *.bgeo with Partio (JB)
            mn::IO::insert_job([&]() {
              mn::write_partio_gridTarget<float, 10>("gridTarget.bgeo",
                                          h_gridTarget);
            });
            mn::IO::flush();
            ...
            initGridTarget(h_gridTarget, h_point_a, h_point_b, target_freq);
...
int main(int argc, char *argv[]) {
  ...
  if (false) {
    ...
    std::vector<std::array<float, 10>> h_gridTarget(mn::config::g_target_cells, std::array<float, 10>{0.f,0.f,0.f,0.f,0.f,0.f,0.f,0.f,0.f,0.f});

    /// Launch initGrid in gmpm_simulator.cuh (JB)
    benchmark->initGrid(graph);
    benchmark->initGridTarget(h_gridTarget, vec<float, 3>{0.f, 0.f, 0.f}, vec<float, 3>{0.f, 0.f, 0.f}, 1.f);
  }
  //getchar();
