...
__global__ void activate_blocks(uint32_t particleCount, ParticleArray parray,
                                Partition partition) {
  uint32_t parid = blockIdx.x * blockDim.x + threadIdx.x; // Global CUDA thread ID (particle proxy)
  if (parid >= particleCount)
    return; // Return if global CUDA thread ID exceeds particle count
  ivec3 blockid{
      int(std::lround(parray.val(_0, parid) / g_dx) - 2) / g_blocksize,
      int(std::lround(parray.val(_1, parid) / g_dx) - 2) / g_blocksize,
      int(std::lround(parray.val(_2, parid) / g_dx) - 2) / g_blocksize}; // Block coord. [i,j,k]
  partition.insert(blockid); // Add blockid to partitions, increments _cnt
}
...
build_particle_cell_buckets(uint32_t particleCount, ParticleArray parray,
  ...
  ivec3 coord{int(std::lround(parray.val(_0, parid) / g_dx) - 2),
              ...
              int(std::lround(parray.val(_2, parid) / g_dx) - 2)}; // Cell coord. [i,j,k]
  int cellno = (coord[0] & g_blockmask) * g_blocksize * g_blocksize +
               (coord[1] & g_blockmask) * g_blocksize +
               (coord[2] & g_blockmask); // Cell number
  coord = coord / g_blocksize; // Block coord. [i,j,k]
  auto blockno = partition.query(coord); // Block number
  // Incr. _ppcs for this cell on particle buffer, set particle-ID-in-cell to this value
  auto pidic = atomicAdd(pbuffer._ppcs + blockno * g_blockvolume + cellno, 1);
  // Set particle-buffer._cellbuckets to global (model) particle ID, will hold all particle IDs arranged in cell buckets
  // i.e. every grid-cell has a bucket, _cellbuckets[x] (model specific) holds the arranged particle IDs for that cell
  // i.e. know the grid-cell and you know what specific particles are inside it easily
...
__global__ void cell_bucket_to_block(int *_ppcs, int *_cellbuckets, int *_ppbs,
                                     int *_buckets) {
  // Each CUDA block is a grid-block
  // Each CUDA thread is a grid-cell
  int cellno = threadIdx.x & (g_blockvolume - 1); // Cell number in grid-block
  int pcnt = _ppcs[blockIdx.x * g_blockvolume + cellno]; // Particle-count in cell
  // For a thread, Loop over particles in cell, limit by max PPC value (settings.h)
  for (int pidic = 0; pidic < g_max_ppc; pidic++) {
    // If particle-count in cell not exceeded
    if (pidic < pcnt) {
      // Incr. _ppbs for block on particle-buffer, set particle-ID-in-block to value
      auto pidib = atomicAggInc<int>(_ppbs + blockIdx.x); 
      // Fill _blockBuckets at appropiate block with particle IDs from _cellBuckets
...
__global__ void compute_bin_capacity(uint32_t blockCount, int const *_ppbs,
                                     int *_bincaps) {
  uint32_t blockno = blockIdx.x * blockDim.x + threadIdx.x; // Global grid-block number
  if (blockno >= blockCount)
    return; // Return if global grid-block number exceeds inputted limit (blockCount)
  // From particles-per-block (on particle-buffer), calc. minimum particle bins for a grid-block 
...
// Clear (zero) each grid-cell in a given grid (grid-buffer)
// Kernel launched by reset() in grid_buffer.cuh, 
// which is called by gmpm_simulator.cuh on a grid-buffer data-structure
template <typename Grid> __global__ void clear_grid(Grid grid) {
  auto gridblock = grid.ch(_0, blockIdx.x); //< CUDA block sets grid-buffer block
  // Loop over cells in grid-block
  for (int cidib = threadIdx.x; cidib < g_blockvolume; cidib += blockDim.x) {
    // Zero-out cell-values of given grid-block (in grid-buffer)
    gridblock.val_1d(_0, cidib) = 0.f; //< Mass
    gridblock.val_1d(_1, cidib) = 0.f; //< Velocity-x
    gridblock.val_1d(_2, cidib) = 0.f; //< Velocity-y
    gridblock.val_1d(_3, cidib) = 0.f; //< Velocity-z
  }
}
...
__global__ void array_to_buffer(ParticleArray parray,
  ...
  int pcnt = pbuffer._ppbs[blockno]; // Particle-count in the block
  auto bucket = pbuffer._blockbuckets + blockno * g_particle_num_per_block; // Block-bucket start, contains global (model) particle-IDs
  // Loop over particles in the block
  for (int pidib = threadIdx.x; pidib < pcnt; pidib += blockDim.x) {
    auto parid = bucket[pidib]; //< Global (model) particle ID
    auto pbin =
        pbuffer.ch(_0, pbuffer._binsts[blockno] + pidib / g_bin_capacity); //< Start of particle-bin
    
    // Set values of the particle-bin (particle-buffer) by the global particle array
    /// pos
...
__global__ void mark_active_grid_blocks(uint32_t blockCount, const Grid grid,
  ...
  if (blockno >= blockCount)
    return;
  // If mass at grid-cell is not zero (i.e. particles are present)
  if (grid.ch(_0, blockno).val_1d(_0, cellno) != 0.f)
    _marks[blockno] = 1; // Mark grid-block the cell belongs to as active
}
...
__global__ void mark_active_particle_blocks(uint32_t blockCount,
  ...
  if (blockno >= blockCount)
    return;
  // If there are particles in the block
  if (_ppbs[blockno] > 0)
    _marks[blockno] = 1; // Mark particle-block as active
}
