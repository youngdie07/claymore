  ...
  if (blockno < blockCount) {
    ...
    for (int cidib = threadIdx.x % 32; cidib < g_blockvolume; cidib += 32) {
      ...
      if (mass > 0.f) {
        ...
        float flumex = 0.584f / g_length; // + (2.f * g_dx); // Length
        float flumey = 0.365f / g_length; // Depth
        float flumez = 0.01f / g_length; // Width
        ...
        // struct_pos[0] = (1.f) / g_length + (2.f * g_dx) + offset;
        // struct_pos[1] = (flumey - struct_dim[1]) / 2.f + offset;
        ...
        //   } else {
        //     if (iter != 5) isOnStructFace[iter] = (1 << iter / 2);
        //     else {
        //       if (vel[0] > 0.f) {
        ...
        vel_n[0] = isInFlumeASFLIP & 4 ? vel_n[0] * mass : vel_n[0] * mass; //< vx = mvx / m
        vel_n[1] = isInFlumeASFLIP & 2 ? vel_n[1] * mass : vel_n[1] * mass; //< vy = mvy / m
        vel_n[2] = isInFlumeASFLIP & 1 ? vel_n[2] * mass : vel_n[2] * mass; //< vz = mvz / m
#endif        
...
retrieve_particle_buffer_attributes(Partition partition,
  ...
  for (int pidib = threadIdx.x; pidib < pcnt; pidib += blockDim.x) {
    ...
    if (part_ID == 290){
      float dv = source_bin.val(_0, _source_pidib);
