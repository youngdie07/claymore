      ...
      if (it != doc.MemberEnd()) {
        if (it->value.IsArray()) {
          ...
          for (auto &model : it->value.GetArray()) {
            ...
            //}
            // else {
            //   benchmark->initGridTarget(model["gpu"].GetInt(), 
            //       h_gridTarget, h_point_a, h_point_b, 
            //       model["output_frequency"].GetFloat());
            // }
          }
        }
